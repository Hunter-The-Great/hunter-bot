...
const data = new SlashCommandBuilder()
  ...
  .addSubcommand((subcommand) =>
    subcommand.setName("leaderboard").setDescription("Shows the leaderboard.")
  )
  .addSubcommand((subcommand) =>
    subcommand
      .setName("stats")
  ...
  if (interaction.options.getSubcommand() === "game") {
    ...
    if (!guild.logging) {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (!message) continue;
        if (message && message.content !== "" && !message.author.bot) break;
      ...
      messageLink = `https://discord.com/channels/${interaction.guild!.id}/${
        message.channel
      }/${message.id}`;
    ...
    try {
      ...
      collector.on("collect", async (i) => {
        ...
        await prisma.user.update({
          ...
          data: {
            ...
            guessingPoints: {
              increment: i.customId.split(":")[1] === correctAnswer ? 1 : 0,
            },
          },
        });
      ...
      collector.on("end", async () => {
        ...
        const response = new EmbedBuilder()
          .setColor(0x00ffff)
          .setTitle(`${message.content}`)
          .setDescription(`Author: <@${messageAuthor}>\nLink: ${messageLink}`)
          .addFields(userMap);
    ...
    const response = new EmbedBuilder()
      ...
      .addFields(
        ...
        },
        {
          name: "Accuracy",
          value: `${Number(
            ((user.guessingPoints / user.guessingAttempts) * 100).toFixed(2)
          )}%`,
          inline: true,
        }
      );
