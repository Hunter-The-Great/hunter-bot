...
const executeJarvis = async (message: Message) => {
  try {
    ...
    if (message.content.toLowerCase().match(zooCheck)) {
      ...
      try {
        const zooMessage = await message.fetchReference().catch(async () => {
        ...
        await message.channel.send("Apologies, I'm not sure who you mean.");
      }
    ...
    if (message.content.toLowerCase().match(request)) {
      ...
      if (!result) {
        await message.channel.send("Apologies, I can't seem to find that one.");
        return;
      }
