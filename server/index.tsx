  ...
  server.post(
    ...
    useSchema(ghSchema, async (req, res) => {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          req.body.head_commit.message || "Could not find commit message."
        )
        .addFields([
          {
            name: "Commit",
          ...
          {
            name: "Branch",
            value: req.body.ref || "Could not find branch information.",
            inline: true,
          },
          {
            name: "Timestamp",
            value:
              req.body.head_commit.timestamp || "Could not find timestamp.",
            inline: true,
          },
        ]);
  ...
  server.post(
    ...
    useSchema(loginSchema, async (req, res) => {
      ...
      res.send(
        ...
        >
          <div id="key">
            <input type="hidden" name="key" value={process.env.MESSAGE_KEY} />
          </div>
          <div id="guilds">
            <label>Guild: </label>
            <select
  ...
  server.post("/channel", async (req, res) => {
    ...
    res.send(
      <div id="message" class="flex flex-wrap justify-center items-end">
        <div id="message-input" class="flex flex-wrap justify-center items-end">
          <div id="message-area">
  ...
  server.post(
    ...
    useSchema(messageSchema, async (req, res) => {
      ...
      if (embed) {
        if (!description)
          return res.code(400).send(
            <div id="message-error" class="flex text-red-700 justify-center">
              Cannot send an embed with an empty description.
            </div>
          );
        const messageEmbed = new EmbedBuilder().setDescription(description);
        try {
          ...
          if (timestamp) messageEmbed.setTimestamp(parseDate(timestamp));
      ...
      if (!req.body.embed) {
        ...
        res.send(
          <div id="embed" class="flex flex-col justify-center">
            <label>Title: </label>
            <input type="text" name="title" class="rounded bg-slate-950 p-1" />
            ...
            />
            <label>Color: </label>
            <input type="color" name="color" class="rounded bg-slate-950 p-1" />
            ...
            />
            <label>Image: </label>
            <input type="text" name="image" class="rounded bg-slate-950 p-1" />
